...
# match方法
# re.match(要查找的内容,从哪里找) 尝试从字符串的起始位置匹配一个模式，匹配成功 返回的是一个匹配 对象 ，
# （返回的这个对象包含了我们匹配的信息），如果不是起始位置匹配成功的话， match()返回的是空，
# 注意：match只能匹配到一个
# 【演示】
str_2 = "张身三高：178,体重：168,学号：123456,密码：1024"


# search方法
# re.search(要查找的内容,从哪里找) 扫描整个字符串,匹配成功 返回的是一个匹配对象（这个对象包含了我们匹配的信息）
